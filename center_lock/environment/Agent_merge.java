...
public class Agent_merge{
    ...
    public Agent_merge(int c, int r){
        this.col = c;
        this.row = r;
        this.state = "d";
        this.range = 3;
    ...
    public void dispersion(Grid_merge grid, Agent_merge[] agents){
                        ...
                        if(candidate.size() != 0){
                            ...
                            for(int i = row - range; i <= row + range; i++){
                                for(int j = col - range; j <= col + range; j++){
                                    if(i >= 0 && i < Variable.N && j >= 0 && j < Variable.M){
                                        if(GetDist(row, col, i, j) <= range){
                                            grid.pherData[i][j] = grid.pherData[i][j] + Variable.c * sum_pher;
                                        }
                                    }
                                }
                            }
                            ...
                            for(int i=0; i<Variable.H; i++){
                                for(int j=0; j<Variable.W; j++){
                                    if(grid.agent_pos[i][j] == 0 && grid.table[i][j] == 1){
                                        candidate.add(i*Variable.M+j);
                                    }
                                }
                            }
                            if(candidate.size() != 0){
                                ...
                                grid.agent_pos[row][col] = 0;
                                grid.occupied[row][col] = 0;
                                length_move = length_move + Math.abs(col - a) + Math.abs(row - b);
                                col = a;
                                row = b;
                                if(grid.agent_pos[b][a] == 1){
                                    System.out.println("Invalid in termination point");
    ...
    public void move_dis(Grid_merge grid, int leftEnd, int rightEnd, int upperEnd, int lowerEnd, Agent_merge[] agents){
    ...
    public void check_dis(Grid_merge grid, int leftEnd, int rightEnd, int upperEnd, int lowerEnd, Agent_merge[] agents, int mr, int mc){
    ...
    public void exploration(Grid_merge grid, Agent_merge[] agents){
        int leftEnd, rightEnd;
        int upperEnd, lowerEnd;
        leftEnd = (areaNo % Variable.m) * Variable.W;
        rightEnd = leftEnd + Variable.W - 1;
    ...
    public void move_exp(Grid_merge grid, Agent_merge[] agents){
        for(int i=0; i<Variable.n; i++){
            for(int j=0; j<Variable.m; j++){
    ...
    public void check_exp(Grid_merge grid, Agent_merge[] agents, int mr, int mc){
        ArrayList<Integer> candiList = new ArrayList<Integer>();
        int change_pos;
        //int unyo = 0;
        for(int i = 0; i < Variable.AGENT_NUM; i++){
    ...
    public void dis2(Grid_merge grid, Agent_merge[] agents){
        int leftEnd, rightEnd;
        int upperEnd, lowerEnd;
    ...
    public void move_dis2(Grid_merge grid, int leftEnd, int rightEnd, int upperEnd, int lowerEnd, Agent_merge[] agents){
        // copy pheromone data of the area which this agent exists
        for(int i=upperEnd; i<=lowerEnd; i++){
            for(int j=leftEnd; j<=rightEnd; j++){
    ...
    public void check_dis2(Grid_merge grid, int leftEnd, int rightEnd, int upperEnd, int lowerEnd, Agent_merge[] agents, int mr, int mc){
        ArrayList<Integer> candiList = new ArrayList<Integer>();
        int change_pos;
        for(int i = 0; i < Variable.AGENT_NUM; i++){
            if(this != agents[i]){
    ...
    public void exp2(Grid_merge grid, Agent_merge[] agents){
        int leftEnd, rightEnd;
        int upperEnd, lowerEnd;
        leftEnd = (areaNo % second_m) * second_W;
        rightEnd = leftEnd + second_W - 1;
    ...
    public void move_exp2(Grid_merge grid, Agent_merge[] agents){
        for(int i=0; i<second_n; i++){
            for(int j=0; j<second_m; j++){
    ...
    public void check_exp2(Grid_merge grid, Agent_merge[] agents, int mr, int mc){
        ArrayList<Integer> candiList = new ArrayList<Integer>();
        int change_pos;
        //int unyo = 0;
        for(int i = 0; i < Variable.AGENT_NUM; i++){
