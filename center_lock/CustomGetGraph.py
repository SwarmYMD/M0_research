...
def getGraph():
    ...
    ax.invert_yaxis()
    fig.savefig("./pics_avoid/step0.png")
    plt.close()
    ...
    data_set2 = np.loadtxt(
        fname = "./csv_avoid/step"+str(half_step)+".csv",
        dtype = int,
        delimiter = ","
    )
    for i in range(height):
    ...
    fig.subplots_adjust(left=0, right=1, bottom=0, top=1)
    ax.invert_yaxis()
    fig.savefig("./pics_avoid/step"+str(half_step)+".png")
    plt.close()
    """
    ...
    data_set3 = np.loadtxt(
        fname = "./csv_avoid/step"+str(final_step)+".csv",
        dtype = int,
        delimiter = ","
    )
    for i in range(height):
    ...
    fig.subplots_adjust(left=0, right=1, bottom=0, top=1)
    ax.invert_yaxis()
    fig.savefig("./pics_avoid/step"+str(final_step)+".png")
